...
static constexpr uint16_t DELAY_GPS_GLOBAL_ORIGIN_TOPIC_MS =
  AP_DDS_DELAY_GPS_GLOBAL_ORIGIN_TOPIC_MS;
...
ardupilot_msgs_msg_AttitudeTarget AP_DDS_Client::rx_attitude_control_topic{};
...
const AP_Param::GroupInfo AP_DDS_Client::var_info[]
{
  ...
  // @User: Advanced
  AP_GROUPINFO_FLAGS(
    "_ENABLE", 1, AP_DDS_Client, enabled, ENABLED_BY_DEFAULT, AP_PARAM_FLAG_ENABLE),
    ...
    // @Description: The time in milliseconds the DDS client will wait for a response from the XRCE
    // agent before reattempting.
    // @Units: ms
    // @Range: 1 10000
    // @RebootRequired: True
    // @Increment: 1
    ...
    // @Description: The maximum number of times the DDS client will attempt to ping the XRCE agent
    // before exiting. Set to 0 to allow unlimited retries.
    // @Range: 0 100
    // @RebootRequired: True
    // @Increment: 1
    // @User: Standard
...
#if AP_DDS_STATIC_TF_PUB_ENABLED | AP_DDS_LOCAL_POSE_PUB_ENABLED | AP_DDS_GEOPOSE_PUB_ENABLED | \
  AP_DDS_IMU_PUB_ENABLED
...
#endif  // AP_DDS_STATIC_TF_PUB_ENABLED | AP_DDS_LOCAL_POSE_PUB_ENABLED | AP_DDS_GEOPOSE_PUB_ENABLED
        // | AP_DDS_IMU_PUB_ENABLED
...
void AP_DDS_Client::update_topic(sensor_msgs_msg_BatteryState & msg, const uint8_t instance)
{
  ...
  static_assert(
    AP_BATT_MONITOR_MAX_INSTANCES <= 99, "AP_BATT_MONITOR_MAX_INSTANCES is greater than 99");
  ...
  msg.power_supply_health = (battery.overpower_detected(instance))
                              ? 4
                              : 1;  // POWER_SUPPLY_HEALTH_OVERVOLTAGE or POWER_SUPPLY_HEALTH_GOOD
...
void AP_DDS_Client::update_topic(geometry_msgs_msg_PoseStamped & msg)
{
  ...
  if (ahrs.get_quaternion(orientation)) {
    Quaternion aux(
      orientation[0], orientation[2], orientation[1],
      -orientation[3]);                                               // NED to ENU transformation
...
void AP_DDS_Client::update_topic(geographic_msgs_msg_GeoPoseStamped & msg)
{
  ...
  if (ahrs.get_location(loc)) {
    ...
    // TODO this is assumed to be absolute frame in WGS-84 as per the GeoPose message definition in
    // ROS. Use loc.get_alt_frame() to convert if necessary.
...
bool AP_DDS_Client::update_topic_goal(geographic_msgs_msg_GeoPointStamped & msg)
{
  ...
  const double tolerance_lat_lon =
    1e-8;  // One order of magnitude smaller than the target's resolution.
  const double distance_alt = 1e-3;
  if (
    abs(msg.position.latitude - prev_goal_msg.position.latitude) > tolerance_lat_lon ||
...
void AP_DDS_Client::on_topic_trampoline(
  uxrSession * uxr_session, uxrObjectId object_id, uint16_t request_id, uxrStreamId stream_id,
...
void AP_DDS_Client::on_topic(
  uxrSession * uxr_session, uxrObjectId object_id, uint16_t request_id, uxrStreamId stream_id,
  struct ucdrBuffer * ub, uint16_t length)
{
  /*
  ...
  */
  ...
  switch (object_id.id) {
      ...
      if (rx_joy_topic.axes_size >= 4) {
        ...
        for (uint8_t i = 0; i < MIN(8U, rx_joy_topic.axes_size); i++) {
          ...
          if (std::isnan(rx_joy_topic.axes[i])) {
            ...
            const uint16_t mapped_data = static_cast<uint16_t>(linear_interpolate(
              rc().channel(i)->get_radio_min(), rc().channel(i)->get_radio_max(),
              rx_joy_topic.axes[i], -1.0, 1.0));
    ...
    case topics[to_underlying(TopicIndex::DYNAMIC_TRANSFORMS_SUB)].dr_id.id: {
      const bool success =
        tf2_msgs_msg_TFMessage_deserialize_topic(ub, &rx_dynamic_transforms_topic);
      if (success == false) {
        break;
      }
    ...
    case topics[to_underlying(TopicIndex::ATTITUDE_CONTROL_SUB)].dr_id.id: {
      const bool success =
        ardupilot_msgs_msg_AttitudeTarget_deserialize_topic(ub, &rx_attitude_control_topic);
      if (success == false) {
        break;
      }
#if AP_EXTERNAL_CONTROL_ENABLED
    ...
    case topics[to_underlying(TopicIndex::VELOCITY_CONTROL_SUB)].dr_id.id: {
      const bool success =
        geometry_msgs_msg_TwistStamped_deserialize_topic(ub, &rx_velocity_control_topic);
      if (success == false) {
        break;
      }
#if AP_EXTERNAL_CONTROL_ENABLED
    ...
    case topics[to_underlying(TopicIndex::GLOBAL_POSITION_SUB)].dr_id.id: {
      const bool success =
        ardupilot_msgs_msg_GlobalPosition_deserialize_topic(ub, &rx_global_position_control_topic);
      if (success == false) {
        break;
      }

#if AP_EXTERNAL_CONTROL_ENABLED
      if (!AP_DDS_External_Control::handle_global_position_control(
            rx_global_position_control_topic)) {
 ...
 */
void AP_DDS_Client::on_request_trampoline(
  uxrSession * uxr_session, uxrObjectId object_id, uint16_t request_id, SampleIdentity * sample_id,
...
void AP_DDS_Client::on_request(
  uxrSession * uxr_session, uxrObjectId object_id, uint16_t request_id, SampleIdentity * sample_id,
  ucdrBuffer * ub, uint16_t length)
{
  (void)request_id;
  (void)length;
  switch (object_id.id) {
    ...
    case services[to_underlying(ServiceIndex::ARMING_MOTORS)].rep_id: {
      ...
      const bool deserialize_success =
        ardupilot_msgs_srv_ArmMotors_Request_deserialize_topic(ub, &arm_motors_request);
      if (deserialize_success == false) {
        break;
      }

      GCS_SEND_TEXT(
        MAV_SEVERITY_INFO, "%s Request for %sing received", msg_prefix,
        arm_motors_request.arm ? "arm" : "disarm");
#if AP_EXTERNAL_CONTROL_ENABLED
      const bool do_checks = true;
      arm_motors_response.result =
        arm_motors_request.arm ? AP_DDS_External_Control::arm(AP_Arming::Method::DDS, do_checks)
                               : AP_DDS_External_Control::disarm(AP_Arming::Method::DDS, do_checks);
      ...
      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::ARMING_MOTORS)].rep_id, .type = UXR_REPLIER_ID};
      ...
      const bool serialize_success =
        ardupilot_msgs_srv_ArmMotors_Response_serialize_topic(&reply_ub, &arm_motors_response);
      if (serialize_success == false) {
        break;
      }

      uxr_buffer_reply(
        uxr_session, reliable_out, replier_id, sample_id, reply_buffer,
        ucdr_buffer_length(&reply_ub));
      GCS_SEND_TEXT(
        MAV_SEVERITY_INFO, "%s Request for Arming/Disarming : %s", msg_prefix,
        arm_motors_response.result ? "SUCCESS" : "FAIL");
      break;
    }
#endif  // AP_DDS_ARM_SERVER_ENABLED
#if AP_DDS_MODE_SWITCH_SERVER_ENABLED
    case services[to_underlying(ServiceIndex::MODE_SWITCH)].rep_id: {
      ...
      const bool deserialize_success =
        ardupilot_msgs_srv_ModeSwitch_Request_deserialize_topic(ub, &mode_switch_request);
      if (deserialize_success == false) {
        break;
      }
      mode_switch_response.status =
        AP::vehicle()->set_mode(mode_switch_request.mode, ModeReason::DDS_COMMAND);
      mode_switch_response.curr_mode = AP::vehicle()->get_mode();

      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::MODE_SWITCH)].rep_id, .type = UXR_REPLIER_ID};
      ...
      ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
      const bool serialize_success =
        ardupilot_msgs_srv_ModeSwitch_Response_serialize_topic(&reply_ub, &mode_switch_response);
      if (serialize_success == false) {
        break;
      }
      ...
      uxr_buffer_reply(
        ...
        ucdr_buffer_length(&reply_ub));
      GCS_SEND_TEXT(
        MAV_SEVERITY_INFO, "%s Request for Mode Switch : %s", msg_prefix,
        mode_switch_response.status ? "SUCCESS" : "FAIL");
      break;
    }
#endif  // AP_DDS_MODE_SWITCH_SERVER_ENABLED
#if AP_DDS_VTOL_TAKEOFF_SERVER_ENABLED
    case services[to_underlying(ServiceIndex::TAKEOFF)].rep_id: {
      ...
      const bool deserialize_success =
        ardupilot_msgs_srv_Takeoff_Request_deserialize_topic(ub, &takeoff_request);
      if (deserialize_success == false) {
        break;
      }
      ...
      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::TAKEOFF)].rep_id, .type = UXR_REPLIER_ID};
      ...
      ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
      const bool serialize_success =
        ardupilot_msgs_srv_Takeoff_Response_serialize_topic(&reply_ub, &takeoff_response);
      if (serialize_success == false) {
        break;
      }
      ...
      uxr_buffer_reply(
        ...
        ucdr_buffer_length(&reply_ub));
      GCS_SEND_TEXT(
        MAV_SEVERITY_INFO, "%s Request for Takeoff : %s", msg_prefix,
        takeoff_response.status ? "SUCCESS" : "FAIL");
      break;
    }
#endif  // AP_DDS_VTOL_TAKEOFF_SERVER_ENABLED
#if AP_DDS_ARM_CHECK_SERVER_ENABLED
    case services[to_underlying(ServiceIndex::PREARM_CHECK)].rep_id: {
      ...
      const bool deserialize_success =
        std_srvs_srv_Trigger_Request_deserialize_topic(ub, &prearm_check_request);
      if (deserialize_success == false) {
        break;
      }
      prearm_check_response.success = AP::arming().pre_arm_checks(false);
      STRCPY(
        prearm_check_response.message,
        prearm_check_response.success ? "Vehicle is Armable" : "Vehicle is Not Armable");

      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::PREARM_CHECK)].rep_id, .type = UXR_REPLIER_ID};
      ...
      ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
      const bool serialize_success =
        std_srvs_srv_Trigger_Response_serialize_topic(&reply_ub, &prearm_check_response);
      if (serialize_success == false) {
        break;
      }
    ...
    case services[to_underlying(ServiceIndex::SET_PARAMETERS)].rep_id: {
      const bool deserialize_success =
        rcl_interfaces_srv_SetParameters_Request_deserialize_topic(ub, &set_parameter_request);
      if (deserialize_success == false) {
        GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR, "%s Set Parameters Request : Failed to deserialize request.",
          msg_prefix);
        break;
      }
      ...
      for (size_t i = 0; i < set_parameter_request.parameters_size; i++) {
        ...
        if (vp == nullptr || param_isnan || param_isinf) {
          ...
          strncpy(
            set_parameter_response.results[i].reason, "Parameter not found",
        ...
        if ((parameter_flags & AP_PARAM_FLAG_INTERNAL_USE_ONLY) || vp->is_read_only()) {
          set_parameter_response.results[i].successful = false;
          strncpy(
            set_parameter_response.results[i].reason, "Parameter is read only",
            sizeof(set_parameter_response.results[i].reason));
          continue;
        }
        ...
        strncpy(
          set_parameter_response.results[i].reason, "Parameter accepted",
          sizeof(set_parameter_response.results[i].reason));
      }

      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::SET_PARAMETERS)].rep_id, .type = UXR_REPLIER_ID};

      const uint32_t reply_size =
        rcl_interfaces_srv_SetParameters_Response_size_of_topic(&set_parameter_response, 0U);
      ...
      const bool serialize_success = rcl_interfaces_srv_SetParameters_Response_serialize_topic(
        &reply_ub, &set_parameter_response);
      if (serialize_success == false) {
        break;
      }
      ...
      GCS_SEND_TEXT(
        successful_params ? MAV_SEVERITY_INFO : MAV_SEVERITY_WARNING,
        "%s Set Parameters Request : %s", msg_prefix,
        successful_params ? "SUCCESSFUL" : "ONE OR MORE PARAMS FAILED");
      break;
    }
    case services[to_underlying(ServiceIndex::GET_PARAMETERS)].rep_id: {
      const bool deserialize_success =
        rcl_interfaces_srv_GetParameters_Request_deserialize_topic(ub, &get_parameters_request);
      if (deserialize_success == false) {
        GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR, "%s Get Parameters Request : Failed to deserialize request.",
          msg_prefix);
        break;
      }
      ...
      const uxrObjectId replier_id = {
        .id = services[to_underlying(ServiceIndex::GET_PARAMETERS)].rep_id, .type = UXR_REPLIER_ID};

      const uint32_t reply_size =
        rcl_interfaces_srv_GetParameters_Response_size_of_topic(&get_parameters_response, 0U);
      uint8_t reply_buffer[reply_size];
      memset(reply_buffer, 0, reply_size * sizeof(uint8_t));
      ucdrBuffer reply_ub;

      ucdr_init_buffer(&reply_ub, reply_buffer, reply_size);
      const bool serialize_success = rcl_interfaces_srv_GetParameters_Response_serialize_topic(
        &reply_ub, &get_parameters_response);
      if (serialize_success == false) {
        break;
      }
      ...
      GCS_SEND_TEXT(
        successful_read ? MAV_SEVERITY_INFO : MAV_SEVERITY_WARNING,
        "%s Get Parameters Request : %s", msg_prefix,
        successful_read ? "SUCCESSFUL" : "ONE OR MORE PARAM NOT FOUND");
      break;
    }
  ...
  while (true) {
    ...
    while (connected) {
      ...
      if (num_pings_missed > 2) {
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s No ping response, disconnecting", msg_prefix);
        connected = false;
      }
    }
...
bool AP_DDS_Client::init_session()
{
  ...
  best_effort_out =
    uxr_create_output_best_effort_stream(&session, output_reliable_stream, DDS_BUFFER_SIZE);
...
bool AP_DDS_Client::create()
{
  ...
  const auto participant_req_id = uxr_buffer_create_participant_bin(
    &session, reliable_out, participant_id, static_cast<uint16_t>(domain_id), participant_name,
    UXR_REPLACE);
  ...
  constexpr uint16_t requestTimeoutParticipantMs =
    (uint16_t)nRequestsParticipant * maxTimeMsPerRequestMs;
  uint8_t statusParticipant[nRequestsParticipant];
  if (!uxr_run_session_until_all_status(
        &session, requestTimeoutParticipantMs, requestsParticipant, statusParticipant,
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(topics); i++) {
    // Topic
    const uxrObjectId topic_id = {.id = topics[i].topic_id, .type = UXR_TOPIC_ID};
    const auto topic_req_id = uxr_buffer_create_topic_bin(
      &session, reliable_out, topic_id, participant_id, topics[i].topic_name, topics[i].type_name,
      UXR_REPLACE);
    ...
    if (topics[i].topic_rw == Topic_rw::DataWriter) {
      // Publisher
      const uxrObjectId pub_id = {.id = topics[i].pub_id, .type = UXR_PUBLISHER_ID};
      const auto pub_req_id = uxr_buffer_create_publisher_bin(
        &session, reliable_out, pub_id, participant_id, UXR_REPLACE);

      // Data Writer
      const auto dwriter_req_id = uxr_buffer_create_datawriter_bin(
        &session, reliable_out, topics[i].dw_id, pub_id, topic_id, topics[i].qos, UXR_REPLACE);
      ...
      if (!uxr_run_session_until_all_status(
            &session, requestTimeoutMs, requests, status, nRequests)) {
        GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR, "%s Topic/Pub/Writer session request failure for index '%u'",
          msg_prefix, i);
        ...
        GCS_SEND_TEXT(
          MAV_SEVERITY_INFO, "%s Topic/Pub/Writer session pass for index '%u'", msg_prefix, i);
      }
    } else if (topics[i].topic_rw == Topic_rw::DataReader) {
      // Subscriber
      const uxrObjectId sub_id = {.id = topics[i].sub_id, .type = UXR_SUBSCRIBER_ID};
      const auto sub_req_id = uxr_buffer_create_subscriber_bin(
        &session, reliable_out, sub_id, participant_id, UXR_REPLACE);

      // Data Reader
      const auto dreader_req_id = uxr_buffer_create_datareader_bin(
        &session, reliable_out, topics[i].dr_id, sub_id, topic_id, topics[i].qos, UXR_REPLACE);
      ...
      if (!uxr_run_session_until_all_status(
            &session, requestTimeoutMs, requests, status, nRequests)) {
        GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR, "%s Topic/Sub/Reader session request failure for index '%u'",
          msg_prefix, i);
        for (uint8_t s = 0; s < nRequests; s++) {
          GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Status '%d' result '%u'", msg_prefix, s, status[s]);
        }
        ...
        GCS_SEND_TEXT(
          MAV_SEVERITY_INFO, "%s Topic/Sub/Reader session pass for index '%u'", msg_prefix, i);
        uxr_buffer_request_data(
          &session, reliable_out, topics[i].dr_id, reliable_in, &delivery_control);
      }
    }
  }
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(services); i++) {
    ...
    if (services[i].service_rr == Service_rr::Replier) {
      const uxrObjectId rep_id = {.id = services[i].rep_id, .type = UXR_REPLIER_ID};
      const auto replier_req_id = uxr_buffer_create_replier_bin(
        &session, reliable_out, rep_id, participant_id, services[i].service_name,
        services[i].request_type, services[i].reply_type, services[i].request_topic_name,
      ...
      if (!uxr_run_session_until_all_status(&session, requestTimeoutMs, &request, &status, 1)) {
        GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR, "%s Service/Replier session request failure for index '%u'",
          msg_prefix, i);
        ...
        GCS_SEND_TEXT(
          MAV_SEVERITY_INFO, "%s Service/Replier session pass for index '%u'", msg_prefix, i);
...
void AP_DDS_Client::write_time_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::TIME_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_nav_sat_fix_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::NAV_SAT_FIX_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_static_transforms()
{
  ...
  if (connected) {
    ...
    const uint32_t topic_size =
      tf2_msgs_msg_TFMessage_size_of_topic(&tx_static_transforms_topic, 0);
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::STATIC_TRANSFORMS_PUB)].dw_id,
      &ub, topic_size);
...
void AP_DDS_Client::write_local_pose_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::LOCAL_POSE_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_tx_local_velocity_topic()
{
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
      geometry_msgs_msg_TwistStamped_size_of_topic(&tx_local_velocity_topic, 0);
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::LOCAL_VELOCITY_PUB)].dw_id, &ub,
      topic_size);
    const bool success =
      geometry_msgs_msg_TwistStamped_serialize_topic(&ub, &tx_local_velocity_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize");
    }
...
void AP_DDS_Client::write_tx_local_airspeed_topic()
{
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
      ardupilot_msgs_msg_Airspeed_size_of_topic(&tx_local_airspeed_topic, 0);
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::LOCAL_AIRSPEED_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_tx_local_rc_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::LOCAL_RC_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_geo_pose_topic()
{
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
      geographic_msgs_msg_GeoPoseStamped_size_of_topic(&geo_pose_topic, 0);
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::GEOPOSE_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_clock_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::CLOCK_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_gps_global_origin_topic()
{
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
      geographic_msgs_msg_GeoPointStamped_size_of_topic(&gps_global_origin_topic, 0);
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::GPS_GLOBAL_ORIGIN_PUB)].dw_id,
      &ub, topic_size);
    const bool success =
      geographic_msgs_msg_GeoPointStamped_serialize_topic(&ub, &gps_global_origin_topic);
    if (!success) {
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize");
    }
  }
...
void AP_DDS_Client::write_goal_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::GOAL_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_status_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::STATUS_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::update()
{
  ...
  if (cur_time_ms - last_battery_state_time_ms > DELAY_BATTERY_STATE_TOPIC_MS) {
    for (uint8_t battery_instance = 0; battery_instance < AP_BATT_MONITOR_MAX_INSTANCES;
         battery_instance++) {
